 ...
 */
#define STM_CR       0x00
#define STM_CR_CPS   GENMASK(15, 8)
...
static ALWAYS_INLINE void stm_disable_channel(const struct nxp_s32_sys_timer_config *config,
					      uint8_t channel)
{
	REG_WRITE(STM_CCR(channel), FIELD_PREP(STM_CCR_CEN, 0U));
	REG_WRITE(STM_CIR(channel), FIELD_PREP(STM_CIR_CIF, 1U));
}
...
static void stm_isr(const struct device *dev)
{
	...
	for (channel = 0; channel < SYS_TIMER_NUM_CHANNELS; ++channel) {
		pending = FIELD_GET(STM_CCR_CEN, REG_READ(STM_CCR(channel))) &&
			  FIELD_GET(STM_CIR_CIF, REG_READ(STM_CIR(channel)));
...
static int nxp_s32_sys_timer_start(const struct device *dev)
{
	...
	REG_WRITE(STM_CR, REG_READ(STM_CR) | STM_CR_TEN);
				       ...
				       const struct counter_alarm_cfg *alarm_cfg)
{
	...
	REG_WRITE(STM_CCR(channel), FIELD_PREP(STM_CCR_CEN, 1U));
...
static uint32_t nxp_s32_sys_timer_get_pending_int(const struct device *dev)
{
	...
	for (i = 0; i < counter_get_num_of_channels(dev); i++) {
		if (REG_READ(STM_CIR(i)) & STM_CIR_CIF) {
			return 1;
		}
	}
...
static int nxp_s32_sys_timer_init(const struct device *dev)
{
	...
	REG_WRITE(STM_CNT, 0U);
	REG_WRITE(STM_CR,
		  FIELD_PREP(STM_CR_FRZ, config->freeze) |
		  FIELD_PREP(STM_CR_CPS, config->prescaler) |
		  FIELD_PREP(STM_CR_TEN, 1U));
	...
	for (i = 0; i < counter_get_num_of_channels(dev); i++) {
		...
		REG_WRITE(STM_CCR(i), FIELD_PREP(STM_CCR_CEN, 0U));
		REG_WRITE(STM_CIR(i), FIELD_PREP(STM_CIR_CIF, 1U));
